...
## R5.3 update: serial apply path (`filled rows only`) в хабе «Движения»

R5.3 фиксирует канонический submit-контракт UI-уровня без ad hoc write-логики:

- UI формирует список участвующих строк по правилу `qtyInput != ''`;
- участвуют только заполненные строки, пустые игнорируются;
- локальная валидация (`qty > 0`, `unitId`) выполняется на уровне строк до submit;
- при наличии локальных ошибок submit блокируется;
