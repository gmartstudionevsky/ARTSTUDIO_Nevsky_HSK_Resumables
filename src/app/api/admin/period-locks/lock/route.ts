...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const lock = await prisma.periodLock.upsert({
    ...
    create: { year: data.year, month: data.month, isLocked: true, lockedById: user.id, note: data.note ?? null },
    update: { isLocked: true, lockedById: user.id, lockedAt: new Date(), note: data.note ?? null },
