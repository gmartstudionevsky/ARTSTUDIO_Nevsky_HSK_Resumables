...
export function mapStockProjectionRow(row: StockRow): StockListItem {
  ...
  } as StockListItem & { projectionEligibility: typeof invariants.projectionEligibility };
}

export async function getStockProjection(rawQuery: StockListQuery): Promise<StockListResponse> {
