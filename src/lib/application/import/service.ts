...
import {
  ...
  ImportSyncUseCase,
} from '@/lib/application/import/contracts';
...
function openingBatchId(date: Date): string {
  const rand = Math.floor(Math.random() * 9000 + 1000);
  const ymd = date.toISOString().slice(0, 10).replace(/-/g, '');
  return `OPENING-${ymd}-${rand}`;
}
...
async function loadDraftPayload(
  ...
  jobId: string,
  userId: string,
  status: ImportJobStatus = ImportJobStatus.DRAFT,
...
): Map<number, { action: 'AUTO' | 'CREATE' | 'SKIP'; accountingPositionId?: string }> {
  const decisionsMap = new Map<
    number,
    { action: 'AUTO' | 'CREATE' | 'SKIP'; accountingPositionId?: string }
  >();
  for (const row of payload.syncPlan?.rows ?? []) {
    if (row.status === 'MATCHED' && (row.selectedAccountingPositionId ?? row.selectedItemId)) {
      decisionsMap.set(row.rowNumber, {
        action: 'AUTO',
        accountingPositionId: row.selectedAccountingPositionId ?? row.selectedItemId ?? undefined,
      });
    }
    if (row.status === 'SKIP') decisionsMap.set(row.rowNumber, { action: 'SKIP' });
  }
  for (const decision of options?.decisions ?? []) {
    decisionsMap.set(decision.rowNumber, {
      action: decision.action,
      accountingPositionId: decision.accountingPositionId ?? decision.itemId,
    });
  }
  return decisionsMap;
}
...
export function createImportSyncUseCase(
  deps: ImportSyncDeps = {
    db: prisma,
    parseImportWorkbook,
    validateImportData,
  ...
  return {
    async previewFromWorkbook(input): Promise<ImportPreviewResult> {
      ...
      return {
        ...
        openingSemantics: {
          ...
          defaultMode: 'OPENING',
          assumption:
            'Строки из колонки остатков/склада будут проведены как инвентаризация открытия без прихода/расхода.',
        },
    ...
    async apply(command: ImportApplyCommand): Promise<ImportApplyResult> {
      ...
      const openingDate = command.options?.openingDate
        ? new Date(command.options.openingDate)
        : new Date();
      if (Number.isNaN(openingDate.getTime())) throw new Error('Некорректная дата начала учета.');
      const payload = await loadDraftPayload(deps.db, command.jobId, command.userId);
      if (payload.errors.length > 0)
        throw new Error('Импорт содержит ошибки. Исправьте файл и повторите предпросмотр.');
      ...
      const txResult = await deps.db.$transaction(
        async (tx) => {
          ...
          if (createOpening) {
            const existingOpening = await tx.transaction.findFirst({
              where: {
                batchId: {
                  startsWith: `OPENING-${openingDate.toISOString().slice(0, 10).replace(/-/g, '')}`,
                },
          ...
          const sections = [
            ...new Set(payload.rows.directory.map((row) => row.sectionCode)),
          ].filter(Boolean);
          const expenseArticleCodes = [
            ...new Set(payload.rows.directory.map((row) => row.expenseArticleCode)),
          ].filter(Boolean);
          ...
          const rollback: RollbackMeta = {
            createdAccountingPositionIds: [],
            createdItemIds: [],
            updatedItems: [],
            openingTransactionId: null,
          };
          const openingLinePayload: Array<{
            itemId: string;
            qtyInput: Prisma.Decimal;
            unitId: string;
          ...
          for (const row of payload.rows.directory) {
            ...
            if (!targetAccountingPositionId && decision?.action === 'AUTO') {
              const planned = payload.syncPlan?.rows?.find(
                (planRow) => planRow.rowNumber === row.rowNumber,
              );
              targetAccountingPositionId =
                planned?.selectedAccountingPositionId ?? planned?.selectedItemId ?? null;
            }
            ...
            const existingItem = targetAccountingPositionId
              ? await tx.accountingPosition.findUnique({
                  where: { id: targetAccountingPositionId },
                  select: { id: true },
                })
              : await tx.accountingPosition.findFirst({
                  where: { OR: [{ code: row.code }, { name: row.name, categoryId }] },
                  select: { id: true },
                });
            ...
            if (existingItem) {
              ...
              const snapshotUnits = await tx.accountingPositionUnit.findMany({
              ...
              const generatedCode =
                row.code ||
                (await deps.generateNextItemCode(tx as Parameters<typeof generateNextItemCode>[0]));
            ...
            >();
            dedup.set(baseUnitId, {
              factorToBase: toDecimal(1),
              isAllowed: true,
              isDefaultInput: defaultInputUnitId === baseUnitId,
              isDefaultReport: reportUnitId === baseUnitId,
            });
            ...
            if (!dedup.has(defaultInputUnitId))
              dedup.set(defaultInputUnitId, {
                factorToBase: toDecimal(1),
                isAllowed: true,
                isDefaultInput: true,
                isDefaultReport: false,
              });
            if (!dedup.has(reportUnitId))
              dedup.set(reportUnitId, {
                factorToBase: toDecimal(1),
                isAllowed: true,
            ...
            const rowsToCreate = Array.from(dedup.entries()).map(([unitId, value]) => ({
          ...
          if (createOpening && openingLinePayload.length > 0) {
            const txOpening = await tx.transaction.create({
              data: {
                batchId: openingBatchId(openingDate),
                type: MovementType.OPENING,
                occurredAt: openingDate,
                note: `Инвентаризация стартовых остатков (${openingDate.toISOString().slice(0, 10)}) (Import #${command.jobId})`,
            ...
            await tx.transactionLine.createMany({
              data: openingLinePayload.map((line) => ({
                ...line,
                transactionId: txOpening.id,
                status: RecordStatus.ACTIVE,
              })),
            });
          ...
          return {
            ...
            touchedAccountingPositionIds: Array.from(touchedAccountingPositionIds),
          };
        },
      ...
      return {
        ...
        warnings: payload.warnings,
        opening: {
          created: txResult.openingCreated,
          mode: txResult.openingType,
          lines: created.openingLines,
        },
        projections,
    ...
    async rollback(input): Promise<ImportRollbackResult> {
      const job = await deps.db.importJob.findFirst({
        where: { id: input.jobId, createdById: input.userId, status: ImportJobStatus.COMMITTED },
      });
      ...
      if (!rollback)
        throw new Error(
          'Для этого импорта откат недоступен. Выполните новый импорт, чтобы включить rollback-метаданные.',
        );
        ...
        async (tx) => {
          if (rollback.openingTransactionId) {
            await tx.transactionLine.deleteMany({
              where: { transactionId: rollback.openingTransactionId },
            });
          ...
          for (const accountingPositionId of rollback.createdAccountingPositionIds) {
            const linesCount = await tx.transactionLine.count({
              where: { itemId: accountingPositionId },
            });
            if (linesCount > 0)
          ...
          for (const item of rollback.updatedItems) {
            ...
            await tx.accountingPosition.update({
              ...
              data: {
                ...
                minQtyBase:
                  item.before.minQtyBase === null
                    ? null
                    : new Prisma.Decimal(item.before.minQtyBase),
