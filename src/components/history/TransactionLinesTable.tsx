          ...
          {lines.map((line) => (
            <tr key={line.id} className={`border-t border-border align-top ${line.status === 'CANCELLED' ? 'bg-surface/60 text-muted' : ''}`}>
              <td className="px-4 py-3">
                <Link className="text-accent underline" href={`/items/${line.accountingPosition.id}`}>{line.accountingPosition.name}</Link>
              ...
              <td className="px-4 py-3">{line.expenseArticle.code} — {line.expenseArticle.name}<br />{line.section.code} — {line.section.name}</td>
              <td className="px-4 py-3">
