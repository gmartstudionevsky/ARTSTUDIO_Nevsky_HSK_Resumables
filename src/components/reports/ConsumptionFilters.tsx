  ...
  return (
    <div className="space-y-3 rounded-lg border border-border bg-surface p-4">
      ...
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secondary" size="sm" onClick={() => onPreset('7d')}>7 дней</Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => onPreset('30d')}>30 дней</Button>
        <Button type="button" variant="secondary" size="sm" onClick={() => onPreset('month')}>Текущий месяц</Button>
        {canExport ? (
          <>
            <Button type="button" variant="ghost" size="sm" onClick={() => onExport('csv')}>Скачать CSV</Button>
            <Button type="button" variant="ghost" size="sm" onClick={() => onExport('xlsx')}>Скачать XLSX</Button>
          </>
        ) : null}
      </div>
    </div>
