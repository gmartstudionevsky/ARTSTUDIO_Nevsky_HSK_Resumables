...
export function DistributePurposesModal({ open, totalQty, purposes, initial, onClose, onSave }: { open: boolean; totalQty: string; purposes: LookupItem[]; initial: DistributionDraft[]; onClose: () => void; onSave: (rows: DistributionDraft[]) => void }): JSX.Element | null {
  const [rows, setRows] = useState<DistributionDraft[]>(initial.length ? initial : [{ sectionId: purposes[0]?.id ?? '', qtyInput: '' }]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-2xl space-y-3 rounded-lg border border-border bg-bg p-4">
        ...
        {rows.map((row, index) => (
          <div key={`${index}-${row.sectionId}`} className="grid gap-2 md:grid-cols-2">
            <Select value={row.sectionId} onChange={(event) => setRows((prev) => prev.map((item, idx) => idx === index ? { ...item, sectionId: event.target.value } : item))}>
        ...
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => setRows((prev) => [...prev, { sectionId: purposes[0]?.id ?? '', qtyInput: '' }])}>Добавить строку</Button>
