...
interface CatalogFiltersProps {
  ...
  onChange: (patch: Partial<{ q: string; categoryId: string; expenseArticleId: string; active: 'true' | 'false' | 'all' }>) => void;
}

export function CatalogFilters(props: CatalogFiltersProps): JSX.Element {
  return (
    <div className="grid gap-3 md:grid-cols-4">
      <Input label="Поиск" value={props.q} onChange={(event) => props.onChange({ q: event.target.value })} placeholder="Название или синоним" />
