...
Импорт доступен в админке на странице `/admin/import` и работает только с XLSX в фиксированном формате.

Обязательные листы:
- `Справочник`
- `Единицы` (может быть пустым, но лист должен существовать)

Ключевые колонки:
- `Справочник`: Код позиции, Номенклатура, Раздел, Ед. базовая, Ед. учёта (по умолчанию), Ед. отчёта (по умолчанию), Назначение, Мин. количество, Остаток на 01.03.2026, Активно, Синонимы, Комментарий.
- `Единицы`: Код позиции, Ед. изм., Коэффициент к базовой, Доступно, По умолчанию (для ввода), По умолчанию (для отчёта).

Как выполнить импорт:
1. Зайдите под `ADMIN` и откройте `/admin/import`.
2. Загрузите файл `.xlsx` и нажмите «Предпросмотр».
3. Проверьте summary и таблицу ошибок/предупреждений.
4. Если ошибок нет — нажмите «Импортировать».
5. Опционально отключите чекбокс «Создать открытие склада 01.03.2026» перед commit.

Откат (только dev):
- Выполните `npm run db:reset`, затем `npm run prisma:migrate` и `npm run seed`.

## Data policies
- Настройки доступны в `/admin/settings`.
