...
### Блок 3 — создание позиции с первичным приходом

- В мастере `Номенклатура → Добавить позицию` включите «Товар уже на складе», укажите количество и единицу.
- При сохранении создаётся позиция и сразу транзакция прихода `IN` с примечанием «Первичное поступление при создании позиции».
- После сохранения карточка позиции открывается автоматически, ссылка «Открыть историю прихода» ведёт на созданную операцию.
...
## History & Stock enhancements
- История: быстрые действия в списке (отмена операции и быстрый drawer для исправления строк).
- История: фильтры по пользователю и разделу.
- History/Stock: сохранённые фильтры (применить, переименовать, удалить, сделать default).
