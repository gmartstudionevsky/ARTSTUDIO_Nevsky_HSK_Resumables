...
test('accounting model: OPENING + INVENTORY_APPLY affect stock as specified', async ({ page, request }) => {
  ...
  const itemsRes = await request.get('/api/items?q=ITM-TEST&active=true');
  ...
  const openingCreateRes = await request.post('/api/inventory', {
  ...
  const openingFillRes = await request.post(`/api/inventory/${openingSessionId}/fill`, {
    data: { scope: 'ITEMS', itemIds: [itemId] },
  });
  expect(openingFillRes.ok()).toBeTruthy();

  const openingDetailRes = await request.get(`/api/inventory/${openingSessionId}`);
  ...
  const openingPatchRes = await request.patch(`/api/inventory/${openingSessionId}/lines`, {
    data: { updates: [{ lineId: openingLineId, qtyFactInput: 5, apply: true }] },
  });
  expect(openingPatchRes.ok()).toBeTruthy();

  const openingApplyRes = await request.post(`/api/inventory/${openingSessionId}/apply`, {
    data: {},
  });
  ...
  const openingTxRes = await request.get(`/api/transactions/${openingApplyPayload.transactionId}`);
  ...
  const regularCreateRes = await request.post('/api/inventory', {
  ...
  const regularFillRes = await request.post(`/api/inventory/${regularSessionId}/fill`, {
    data: { scope: 'ITEMS', itemIds: [itemId] },
  });
  expect(regularFillRes.ok()).toBeTruthy();

  const regularDetailRes = await request.get(`/api/inventory/${regularSessionId}`);
  ...
  const regularPatchRes = await request.patch(`/api/inventory/${regularSessionId}/lines`, {
    data: { updates: [{ lineId: regularLineId, qtyFactInput: 4, apply: true }] },
  });
  expect(regularPatchRes.ok()).toBeTruthy();

  const regularApplyRes = await request.post(`/api/inventory/${regularSessionId}/apply`, {
    data: {},
  });
  ...
  const regularTxRes = await request.get(`/api/transactions/${regularApplyPayload.transactionId}`);
  ...
  const stockRes = await request.get('/api/stock?q=ITM-TEST');
